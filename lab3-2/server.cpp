...
void wait_shake_hand() {//等待建立连接
    while (1) {
        ...
        if (checksum(recv, 2) != 0 || recv[1] != SHAKE_1)//接受了之后算出校验和不等于0或者接收到的不是shake1
            continue;
...
void wait_wave_hand() {//等待挥手
    while (1) {
        ...
        if (checksum(recv, 2) != 0 || recv[1] != (char)WAVE_1)//校验和不等于0或未接收到wave1
            continue;
        recv[1] = WAVE_2;
        recv[0] = checksum(recv + 1, 1);
...
void recv_message(char* message, int& len_recv) {
    ...
    while (1) {
        while (1) {
            ...
            while (recvfrom(server, recv, piece_len + 4, 0, (sockaddr*)&clientAddr, &lentmp) == SOCKET_ERROR);
            char send[3];
            int flag = 0;
            if (checksum(recv, piece_len + 4) == 0 && (unsigned char)recv[2] == last_order) {
                last_order++;
                flag = 1;
            }
